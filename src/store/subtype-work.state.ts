...
export const useSubtypeWork = create<SubtypeWorkState>((set) => ({
    ...
    getAllSubtypeWorks: async (
        ...
        try {
            ...
            alert("Не удалось получить типы работ");
            console.log(error);
        } finally {
            set({ loading: false });
        }
    ...
    getPlaningSubtypeWorks: async (plotId: number, typeWorkId: number) => {
        try {
            ...
            alert("Не удалось получить плановые типы работ");
            console.log(error);
        } finally {
            set({ loading: false });
        }
    ...
    getSubtypeWorkById: async (id: number) => {
        try {
            ...
            alert("Не удалось получить тип работ по id");
            console.log(error);
        } finally {
            set({ loading: false });
        }
    ...
    getSubtypeWorkByTypeWorkId: async (typeWorkId: number) => {
        try {
            ...
            alert("Не удалось получить тип работ по id виду работ");
            console.log(error);
        } finally {
            set({ loading: false });
        }
    },
    createSubtypeWork: async (
        code: string,
        name: string,
        unitMetering: string,
        typeWorkId: number,
        ...
        try {
            ...
            if (response.status === 200) {
                set((state) => ({
                    subtypeWorks: [
                        ...state.subtypeWorks,
                        response.data as SubtypeWork,
                    ],
                }));
                alert("Тип работ создан");
            }
        } catch (error) {
            alert("Не удалось создать тип работ");
            console.log(error);
        } finally {
            set({ loading: false });
        }
    },
    updateSubtypeWork: async (
        id: number,
        code: string,
        name: string,
        unitMetering: string,
        ...
        try {
            ...
            if (response.status === 200) {
                set((state) => ({
                    ...
                    subtypeWorks: state.subtypeWorks.map((sw) =>
                        sw.id === id ? (response.data as SubtypeWork) : sw,
                    ),
                }));
                alert("Тип работ изменён");
            }
        } catch (error) {
            alert("Не удалось изменить тип работ");
            console.log(error);
        } finally {
            set({ loading: false });
        }
    ...
    deleteSubtypeWork: async (id: number) => {
        try {
            ...
            if (response.status === 200) {
                ...
                alert("Тип работ удалён");
            }
        } catch (error) {
            alert("Не удалось удалить тип работ");
            console.log(error);
        } finally {
            set({ loading: false });
        }
